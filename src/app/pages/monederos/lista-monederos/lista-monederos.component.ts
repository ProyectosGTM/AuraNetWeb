...
export class ListaMonederosComponent {
  ...
  public listaMonederosDisponiblesReemplazar: any[] = [];
  ...
  reemplazarMonedero() {
    this.monederosService.obtenerMonederos().subscribe({
      next: (response) => {
        this.listaMonederosDisponiblesReemplazar = (response?.data || []).map((m: any) => ({
          ...m,
          id: Number(m.id),
          text: `${m.numeroMonedero || ''} - ${m.alias || ''}`.trim()
        }));
  ...
  guardarReemplazarMonedero() {
    ...
    this.monederosService.reemplazarMonedero(payload).subscribe({
      next: () => {
        Swal.fire({
          title: '¡Operación Exitosa!',
          text: 'Se reemplazó el monedero correctamente.',
          icon: 'success',
          background: '#0d121d',
          confirmButtonColor: '#3085d6',
          confirmButtonText: 'Confirmar',
...
@ViewChild('modalHistorialMovimientos', { static: false }) modalHistorialMovimientos!: TemplateRef<any>;

historialMovimientos: any[] = [];
historialMonederoLabel = '';
cargandoHistorial = false;

abrirHistorialMovimientos(rowData: any) {
  const id = Number(rowData?.id);
  this.historialMovimientos = [];
  this.historialMonederoLabel = (rowData?.numeroMonedero || rowData?.alias || '').toString().trim();
  this.cargandoHistorial = true;

  this.modalRef = this.modalService.open(this.modalHistorialMovimientos, {
    size: 'lg',
  ...
  this.monederosService.obtenerHistorialMovimientos(id).subscribe({
    next: (resp) => {
      this.cargandoHistorial = false;
      const rows = Array.isArray(resp?.data) ? resp.data : [];
      this.historialMovimientos = rows.map((m: any) => ({
        ...m,
        tipoMovimientoNombre: m?.tipoMovimiento?.nombre || m?.tipoMovimiento?.codigo || '—',
        tipoMovimientoColor: m?.tipoMovimiento?.color || '#50a5f1',
        tipoSaldoNombre: m?.tipoSaldo?.nombre || m?.tipoSaldo?.codigo || '—',
        tipoSaldoColor: m?.tipoSaldo?.color || '#22c55e',
        contextoOperacionNombre: m?.contextoOperacion?.nombre || m?.contextoOperacion?.codigo || '—'
      }));
    },
    error: (error) => {
      this.cargandoHistorial = false;
      Swal.fire({
        title: '¡Error!',
        text: error?.error?.message || error?.error || 'No se pudo obtener el historial de movimientos.',
        icon: 'error',
        background: '#0d121d',
        confirmButtonColor: '#3085d6',
        confirmButtonText: 'Confirmar',
...
colorBg(hex: string): string {
  const c = (hex || '').trim();
  if (!c.startsWith('#') || (c.length !== 7 && c.length !== 4)) return 'rgba(80,165,241,0.12)';
...
cerrarModal() {
  if (this.modalRef) {
    ...
    this.historialMovimientos = [];
    this.historialMonederoLabel = '';
    this.cargandoHistorial = false;
  }
}

// lista-monederos.component.ts (AGREGA)
@ViewChild('modalSaldosMonedero', { static: false }) modalSaldosMonedero!: TemplateRef<any>;
...
abrirSaldosMonedero(rowData: any) {
  const id = Number(rowData?.id);
  this.saldosRaw = null;
  this.saldosLista = [];
  this.saldosMonederoLabel = (rowData?.numeroMonedero || rowData?.alias || '').toString().trim();
  this.cargandoSaldos = true;

  this.modalRef = this.modalService.open(this.modalSaldosMonedero, {
    size: 'lg',
    windowClass: 'modal-holder modal-saldos-monedero',
    centered: true,
    backdrop: 'static',
    keyboard: true
  });

  this.monederosService.obtenerSaldosMonedero(id).subscribe({
    next: (resp) => {
      ...
      if (Array.isArray(data)) {
        lista = data;
      } else if (Array.isArray(data?.saldos)) {
        lista = data.saldos;
      } else if (data && typeof data === 'object') {
        lista = Object.keys(data).map((k) => ({
          codigo: k,
          nombre: k,
          monto: data[k]
        }));
      ...
      this.saldosLista = (lista || []).map((s: any) => ({
        ...s,
        nombre: s?.tipoSaldo?.nombre || s?.nombre || s?.codigo,
        codigo: s?.tipoSaldo?.codigo || s?.codigo,
        color: s?.tipoSaldo?.color || s?.color,
        esRedimible: s?.tipoSaldo?.esRedimible ?? s?.esRedimible,
        monto: s?.monto ?? s?.saldo ?? s?.valor ?? 0
      }));
    },
    error: (error) => {
      this.cargandoSaldos = false;
      Swal.fire({
        title: '¡Error!',
        text: error?.error?.message || error?.error || 'No se pudieron obtener los saldos del monedero.',
        icon: 'error',
        background: '#0d121d',
        confirmButtonColor: '#3085d6',
        confirmButtonText: 'Confirmar',
