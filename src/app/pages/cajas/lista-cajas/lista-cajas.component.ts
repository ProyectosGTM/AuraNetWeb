...
@Component({
  ...
  styleUrl: './lista-cajas.component.scss',
...
export class ListaCajasComponent {
  ...
  agregar() {
    this.router.navigateByUrl('/cajas/agregar-caja');
  }

  actualizarCaja(id: Number) {
    this.router.navigateByUrl('/cajas/editar-caja/' + id);
  }

  activar(rowData: any) {
    Swal.fire({
      title: '¡Activar!',
      html: `¿Está seguro que requiere activar esta caja "${rowData.nombre || rowData.codigo}"?`,
      icon: 'warning',
  ...
  desactivar(rowData: any) {
    Swal.fire({
      title: '¡Desactivar!',
      html: `¿Está seguro que requiere desactivar esta caja "${rowData.nombre || rowData.codigo}"?`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
  ...
  setupDataSource() {
    ...
    this.listaCajas = new CustomStore({
      key: 'id',
      load: async (loadOptions: any) => {
        ...
        try {
          const resp: any = await lastValueFrom(
            this.cajasService.obtenerCajasData(page, take)
          );
          this.loading = false;
  ...
  onGridOptionChanged(e: any) {
    ...
    if (!texto) {
      this.filtroActivo = '';
      grid?.option('dataSource', this.listaCajas);
      return;
    }
